...
const compareCardToDrum = () => {
  for (let i = 0; i <= drum.length - 1; i++) {
    ...
    const newBall = confirm(
      `The ball that comes out of the drum is: ${drum[i]}. Continue?`
    );
    roundCounter();
...
const checkBingo = () => {
  if (
    line1.every((value, index) => value === line2[index]) &&
    line1.every((value, index) => value === line3[index])
  ) {
    bingoSwitch++;
...
const repeat = () => {
  const repetition = confirm(`Are you happy with this card, ${player}?`);
  if (repetition === true) {
    return;
  }
...
const replay = () => {
  const repetition = confirm(`${player}, do you want to start a new game?`);
  if (repetition === true) {
    cardRandom = [];
    line1 = [];
    line2 = [];
...
const randomRounds = () => {
  const rounds = Math.floor(Math.random() * (max - 15 + 1) + 15);
  return rounds;
};
