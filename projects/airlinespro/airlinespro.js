...
const showFilteredResults = (item) => {
  item.forEach((flight) => {
    if (flight.scale === true) {
      console.log(
        `Flight with ID nº ${flight.id} from 🛫 ${flight.from} headed to 🛬 ${flight.to} costs ${flight.cost} € and it has a layover.`
      );
    } else {
      console.log(
        `Flight with ID nº ${flight.id} from 🛫 ${flight.from} headed to 🛬 ${flight.to} costs ${flight.cost} € and it's a direct flight.`
      );
    }
  });
};
...
const askForAdminAction = () => {
  ...
  if (adminAction === null) {
    ...
    adminAction.toUpperCase() !== "CREATE" &&
    adminAction.toUpperCase() !== "DELETE"
  ) {
...
const adminActionsDoThis = () => {
  ...
  if (adminActionsResult === "CREATE") {
    ...
    const deleteFlight = +prompt(
      "Insert the ID of the flight you want to delete:"
    );
