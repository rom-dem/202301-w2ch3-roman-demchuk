...
const repeat = () => {
    let repetition = confirm(`Do you want to repeat with new numbers?`)
    if (repetition === true) {
        numbers = [];
        main();
    } else {
...
const getUserNumbers = () => {
    let number;
    do {
        number = prompt('Please input a number to do the calculations. Use a dot for decimals. Press CANCEL to finish.');
        if (number === null) {
            return;
...
const div = () => {
    ...
    console.log(`The division of all numbers = ${thisOperation} \n ****************`);
}
